...
void drawSine(float offset, float speed,  int maxX, int maxY, float waveHeight, int color) {
    bool first = true;;
    int lx = -1, ly = -1;
    double vx = 0;
    float vmax = 2;
    for (int x = 0; x < maxX; ++x) {
        ...
        if (first) {
            ...
            Paint_DrawLine(lx, ly, x, y, color, LINE_STYLE_SOLID, DOT_PIXEL_1X1);
        }
        lx = x;
        ly = y;
    }
...
void frame(int frameCount) {
    ...
    for (int line=0; line < 4; ++line) {
        drawSine(frameCount+line*(PI), .25, maxX, maxY, .65, lineColor[line]);
    }


    char message[32];
    sprintf(message, "%4.2f", refVoltage);
    Paint_DrawString_EN(2, 2, message, &Font24, BLACK, LGRAY);
